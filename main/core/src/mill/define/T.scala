...
object T:
  ...
  export Task.{create => apply}

  implicit def unboxResult[T](result: Result[T])(using TaskContext): T = result match
  ...
  def fail(msg: String)(using TaskContext): Nothing = throw TaskControl.Failure(msg)
  ...
  def log(msg: String)(using ctx: TaskContext): Unit = ctx.logger.log(Level.INFO, msg)
