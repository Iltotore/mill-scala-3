...
def applicativeImpl[T](expr: Expr[TaskContext ?=> T])(using Quotes, Type[T]): Expr[Task[T]] =
    ...
    def recCase(caseDef: CaseDef): Expr[List[Task[?]]] = caseDef match
        case CaseDef(_, Some(guard), thenp) => '{ ${ recTerm(guard) } ++ ${ recTerm(thenp) } }
    ...
    def recTerm(term: Term): Expr[List[Task[?]]] = term match
        ...
        case Apply(TypeApply(Select(Ident("Task"), "apply"), _), List(task)) => '{ List(${ task.asExprOf[Task[?]] }) }
        case Apply(fun, args)                                                => '{ ${ recTerm(fun) } ++ ${ flatMapToExpr(args, recTerm) } }
        case TypeApply(fun, _)                                               => recTerm(fun)
        case Super(qual, _)                                                  => recTerm(qual)
        case Assign(lhs, rhs)                                                => '{ ${ recTerm(lhs) } ++ ${ recTerm(rhs) } }
        case Block(statements, expr)                                         => '{ ${ flatMapToExpr(statements, recStatement) } ++ ${ recTerm(expr) } }
        case Closure(meth, _)                                                => recTerm(meth)
        case If(cond, thenp, elsep)                                          => '{ ${ recTerm(cond) } ++ ${ recTerm(thenp) } ++ ${ recTerm(elsep) } }
        case Match(selector, cases)                                          => '{ ${ flatMapToExpr(cases, recCase) } ++ ${ recTerm(selector) } }
        case SummonFrom(cases)                                               => flatMapToExpr(cases, recCase)
        case Try(expr, cases, finalizer)                                     => '{ ${ recTerm(expr) } ++ ${ flatMapToExpr(cases, recCase) } ++ ${ mapOptionToExpr(finalizer, recTerm) } }
        case Return(expr, _)                                                 => recTerm(expr)
        case Repeated(terms, _)                                              => flatMapToExpr(terms, recTerm)
        case Inlined(tree, defs, expr)                                       => '{ ${ mapOptionToExpr(tree, recTree) } ++ ${ flatMapToExpr(defs, recDef) } ++ ${ recTerm(expr) } }
